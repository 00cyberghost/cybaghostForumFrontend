...
useHead({
    title: 'People i am following - Cybaghost Forum',
    meta: [
    ...
    <div class="w-full md:grid md:grid-cols-8 h-screen">
        ...
        <div class="col-span-7 md:col-span-6">
            ...
            <div class="md:grid md:grid-cols-6 gap-4">
                ...
                <div v-for="following in followings" :key="following" class="col-span-2 dark-card iterator py-1 px-3 w-full mt-2 rounded-xl shadow-lg space-y-1 sm:py-2 sm:flex sm:items-center sm:space-y-0 sm:space-x-3">
                    <NuxtLink :to="`/user/${following.username}/${following.user_id}`" class=" no-underline"><img class="block mx-auto h-20 w-20 object-cover rounded-full sm:mx-0 sm:shrink-0" :src="utils.userImage(following.image)" alt="Woman's Face" /></NuxtLink>
                    <div class="text-center space-y-2 items-center">
                        <div class="space-y-0.5">
                        <p class="text-md text-black dark:text-white font-semibold">
                            <NuxtLink :to="`/user/${following.username}/${following.user_id}`" class=" no-underline">{{ following.username }}</NuxtLink>
                        </p>
                        <div class="flex justify-center">
                            <div class="border px-2 py-1 text-xs">
                                <ClientOnly><font-awesome-icon class="" icon="fa-comment" /></ClientOnly> {{ following.posts + following.replies }}
                            </div>
                            <div class="border px-2 py-1 text-xs">
                            <ClientOnly> <font-awesome-icon class="" icon="fa-heart" /></ClientOnly> {{ following.likes }}
                            </div>
                            <div class="border px-2 py-1 text-xs">
                                <ClientOnly><font-awesome-icon class="" icon="fa-user-plus" /></ClientOnly> {{ following.followers }}
                            </div>
                        </div>
                        </div>
                        <button @click="toggleFollowUser" :user-id="following.user_id" :follower-user-id="following.follower" :name="following.username" class="px-4 py-1 text-sm bg-primary text-white font-semibold rounded-full border border-rose-500 hover:text-rose-600 hover:bg-rose-600 hover:border-transparent focus:outline-none focus:ring-2 focus:ring-rose-600 focus:ring-offset-2">unFollow</button>
                    </div>
